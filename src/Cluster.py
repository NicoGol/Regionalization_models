...
class Graph:
    ...
    def best_k_regions(self,k,limit_vertex, z, sc, split_tree=False):
        T = self.find_MST_full_order_CL()
        if self.talk:
            print('start finding regions')
        t1 = t.time()
        self.MST.find_regions(limit_vertex, z, sc, 0, 0,split_tree=split_tree)
        t2 = t.time()
...
class Tree():
    ...
    def find_regions(self, limit_vertex, n_split, sc, subtree, actual_split, depth=0, split_tree=False, index=0):
        if split_tree and depth == 0:
            Tree.regions = [[] for _ in range(n_split[actual_split][1])]
            Tree.he = [[] for _ in range(n_split[actual_split][1])]
            Tree.dict_regions = [[] for _ in range(n_split[actual_split][1])]
            Tree.dict_he = [[] for _ in range(n_split[actual_split][1])]
        Tree.regions[subtree] += [self.vertex]
        Tree.dict_regions[subtree][index] = self.vertex
        Tree.he[subtree] += [self.h]
        Tree.dict_he[subtree][index] = self.h
        if len(self.vertex) > limit_vertex:
            if actual_split < len(n_split)-1 and depth >= n_split[actual_split+1][0]:
                actual_split += 1
            new_trees = self.tree_partitioning(n_split[actual_split][1],sc)
            new_indices = list(range(Tree.index_node+1,Tree.index_node+1+len(new_trees)))
            Tree.arch[index] = new_indices
            Tree.index_node += len(new_trees)
            Tree.n_partitions += 1
            for i,tree in enumerate(new_trees):
                if split_tree and depth == 0:
                    tree.find_regions(limit_vertex, n_split, sc, int(i/2), actual_split, depth=depth+1, index=new_indices[i])
                else:
                    tree.find_regions(limit_vertex, n_split, sc, subtree, actual_split, depth=depth+1, index=new_indices[i])
        else:
            Tree.leaves += [depth]
